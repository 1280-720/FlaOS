...
INT show_sysinfo_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (idx = 0; idx < WDEV_NUM_MAX; idx++) {
		...
		if (pAd->wdev_list[idx])
		{
			...
			DBGPRINT(RT_DEBUG_INFO, ("\t\tWdev(list) Idx:%d\n", wdev->wdev_idx));
			...
			DBGPRINT(RT_DEBUG_INFO, ("\t\tPortSecured/AllowTx: %d(%sSecured):%d\n",
						wdev->PortSecured,
						(wdev->PortSecured == WPA_802_1X_PORT_SECURED ? "" : "Not"),
						wdev->allow_data_tx));
			DBGPRINT(RT_DEBUG_INFO, ("\t\ttx_pkt_checker:%p\n", wdev->tx_pkt_allowed));
			DBGPRINT(RT_DEBUG_INFO, ("\t\ttx_pkt_handlerer:%p\n", wdev->tx_pkt_handle));
			DBGPRINT(RT_DEBUG_INFO, ("\t\ttx_pkt_hardTransmit:%p\n", wdev->wdev_hard_tx));

			DBGPRINT(RT_DEBUG_INFO, ("\t\tif_dev:0x%p\tfunc_dev:[%d]0x%p\tsys_handle:0x%p\n",
