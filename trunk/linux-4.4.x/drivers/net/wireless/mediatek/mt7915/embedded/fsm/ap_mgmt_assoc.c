	...
	IN BOOLEAN isReassoc)
{
	...
	*/
	...
	if (HAS_HT_CAPS_EXIST(cmm_ies->ie_exists) &&
		...
		if (WMODE_CAP_AC(PhyMode) && (Channel > 14) &&
...
#if !defined(MT7615) && !defined(MT7637) && !defined(MT7622) && !defined(P18) && !defined(MT7663) && !defined(MT7626) && !defined(AXE) && !defined(MT7915)

		if (!IS_MT7615(pAd) && !IS_MT7637(pAd) && !IS_MT7622(pAd) && !IS_P18(pAd) && !IS_MT7663(pAd) && !IS_MT7626(pAd) && !IS_AXE(pAd) && !IS_MT7915(pAd))
			RAInit(pAd, pEntry);
	...
	if (WMODE_CAP_AC(wdev->PhyMode) &&
		...
		return MLME_ASSOC_REJ_DATA_RATE;
			...
			if (pAd->LowRateCtrl.RatePrior) {
				if (wdev->channel < 14 && !HAS_HT_CAPS_EXIST(ie_list->cmm_ies.ie_exists))
					return MLME_UNSPECIFY_FAIL;
#ifdef DOT11_VHT_AC
				if (wdev->channel > 14 && !HAS_VHT_CAPS_EXIST(ie_list->cmm_ies.ie_exists))
					return MLME_UNSPECIFY_FAIL;
#endif /* DOT11_VHT_AC */
	...
	IE_LISTS *ie_lists)
{
	...
	while (((UCHAR *)eid_ptr + eid_ptr->Len + 1) < ((UCHAR *)Fr + MsgLen)) {
		...
		case IE_SUPP_RATES:
			Sanity |= 0x02;
			parse_support_rate_ie(rate, eid_ptr);
			break;

		case IE_EXT_SUPP_RATES:
			parse_support_ext_rate_ie(rate, eid_ptr);
		...
		case IE_WPA:    /* same as IE_VENDOR_SPECIFIC */
			...
			MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("[%d] has_oui: %d\n", pEntry->wcid, pEntry->has_oui));

			/* Record Broadcom OUI in Assoc Request for BF IOT */
			MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): eid_ptr->Octet[0-2] = %x, %x, %x\n"
			...
			if (NdisEqualMemory(eid_ptr->Octet, P2POUIBYTE, sizeof(P2POUIBYTE)) && (eid_ptr->Len >= 4)) {
				if (ie_lists->P2PSubelementLen == 0) {
					...
					if (((ie_lists->P2PSubelementLen) + (eid_ptr->Len + 2)) <= MAX_VIE_LEN) {
						...
						MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: ERROR!! 111 Sum of P2PSubelementLen= %lu, > MAX_VIE_LEN !!\n",
								 __func__, (ie_lists->P2PSubelementLen + (eid_ptr->Len + 2))));
						return FALSE;
					}
				}

				MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" ! ===>P2P - PeerAssocReqSanity  P2P IE Len becomes = %d.   %s\n",
...
#ifdef DOT11_HE_AX
			parse_he_assoc_req_ies((UINT8 *)eid_ptr, ie_lists);
#endif
			switch (*extension_id) {
	...
	IN BOOLEAN isReassoc)
{
...
#if defined(RT_CFG80211_SUPPORT) && defined(HOSTAPD_PMKID_IN_DRIVER_SUPPORT)
	{
		...
		if ((pPMKID != NULL) &&
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: PMK match\n", __func__));
			...
			if (pPMKID == NULL)
				MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: PMKID NULL\n", __func__));
			else
				MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: PMK match\n", __func__));
		}
	}
...
SendAssocResponse:
	...
	if (IS_HE_STA(pEntry->cap.modes) && WMODE_CAP_AX(wdev->PhyMode)
		...
		if ((wlan_config_get_asic_twt_caps(wdev) &&
		    ...
		    (IS_STA_EXT_CAP_TWT(ie_list) &&
		    IS_STA_CAP_TWT(pEntry) &&
		    IS_STA_WITH_TWT_IE(ie_list))) {
			offset = add_assoc_reassoc_rsp_twt_ie(wdev,
				pEntry->wcid,
...
#ifdef CONFIG_HOTSPOT_R2
	/* qosmap IE */
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("entry wcid %d QosMapSupport=%d\n",
	...
	if ((pFtCfg != NULL) && (pFtCfg->FtCapFlag.Dot11rFtEnable)) {
		...
		os_alloc_mem(pAd, (UCHAR **)&rsnxe_ie, sizeof(MAX_LEN_OF_RSNXEIE));
		os_zero_mem(rsnxe_ie, sizeof(MAX_LEN_OF_RSNXEIE));
		rsnxe_ie_len = build_rsnxe_ie(&wdev->SecConfig, rsnxe_ie);

		if (rsnxe_ie_len != 0 && FtInfoBuf->FtIeInfo.MICCtr.field.IECnt)
			FtInfoBuf->FtIeInfo.MICCtr.field.IECnt++;

		if (ie_list->FtInfo.RicInfo.Len && FtInfoBuf->FtIeInfo.MICCtr.field.IECnt)
			FtInfoBuf->FtIeInfo.MICCtr.field.IECnt++;
...
static VOID ap_peer_disassoc_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
#ifdef DOT11W_PMF_SUPPORT
	{
		...
		if (pPmfCfg->UsePMFConnect == TRUE && Fr->Hdr.FC.Wep == 0) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_TRACE, ("drop due to unprotect disassoc frame\n"));
			return;
		}
	}
#endif
