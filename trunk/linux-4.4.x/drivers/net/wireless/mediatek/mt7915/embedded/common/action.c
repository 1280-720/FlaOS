	...
	OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(S, ACT_IDLE, MT2_MLME_S1G_CATE_TWT_TEARDOWN, (STATE_MACHINE_FUNC)mlme_twt_teradown_action);
#ifdef APCLI_SUPPORT
...
VOID MlmeDELBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeAllocateMemory(pAd, &pOutBuffer) != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("BA - MlmeDELBAAction() allocate memory failed.\n"));
		return;
	}
	...
	*(USHORT *)(&Frame.DelbaParm) = cpu2le16(*(USHORT *)(&Frame.DelbaParm));
	...
	MlmeFreeMemory(pOutBuffer);
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s Initiator = %d DELBA sent\n", __func__, pInfo->Initiator));
}
...
VOID StaPublicAction(RTMP_ADAPTER *pAd, BSS_2040_COEXIST_IE *pBssCoexIE, struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ACTION - StaPeerPublicAction  Bss2040Coexist = %x\n", *((PUCHAR)pBssCoexIE)));
...
VOID UpdateBssScanParm(RTMP_ADAPTER *pAd, OVERLAP_BSS_SCAN_IE APBssScan)
{
	...
	if ((pAd->CommonCfg.Dot11BssWidthChanTranDelayFactor < 5) || (pAd->CommonCfg.Dot11BssWidthChanTranDelayFactor > 100)) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11BssWidthChanTranDelayFactor out of range !!!!)\n"));*/
	...
	if ((pAd->CommonCfg.Dot11BssWidthTriggerScanInt < 10) || (pAd->CommonCfg.Dot11BssWidthTriggerScanInt > 900)) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11BssWidthTriggerScanInt out of range !!!!)\n"));*/
	...
	if ((pAd->CommonCfg.Dot11OBssScanPassiveDwell < 5) || (pAd->CommonCfg.Dot11OBssScanPassiveDwell > 1000)) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11OBssScanPassiveDwell out of range !!!!)\n"));*/
	...
	if ((pAd->CommonCfg.Dot11OBssScanActiveDwell < 10) || (pAd->CommonCfg.Dot11OBssScanActiveDwell > 1000)) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11OBssScanActiveDwell out of range !!!!)\n"));*/
	...
	if ((pAd->CommonCfg.Dot11OBssScanPassiveTotalPerChannel < 200) || (pAd->CommonCfg.Dot11OBssScanPassiveTotalPerChannel > 10000)) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11OBssScanPassiveTotalPerChannel out of range !!!!)\n"));*/
	...
	if ((pAd->CommonCfg.Dot11OBssScanActiveTotalPerChannel < 20) || (pAd->CommonCfg.Dot11OBssScanActiveTotalPerChannel > 10000)) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11OBssScanActiveTotalPerChannel out of range !!!!)\n"));*/
	...
	if (pAd->CommonCfg.Dot11OBssScanActivityThre > 100) {
		/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ACT - UpdateBssScanParm( Dot11OBssScanActivityThre out of range !!!!)\n"));*/
		pAd->CommonCfg.Dot11OBssScanActivityThre = 25;
	}

	pAd->CommonCfg.Dot11BssWidthChanTranDelay = (pAd->CommonCfg.Dot11BssWidthTriggerScanInt * pAd->CommonCfg.Dot11BssWidthChanTranDelayFactor);
	/*MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_LOUD,("ACT - UpdateBssScanParm( Dot11BssWidthTriggerScanInt = %d )\n", pAd->CommonCfg.Dot11BssWidthTriggerScanInt));*/
}
...
VOID PeerPublicAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
#endif /* P2P_SUPPORT */
		{
			...
			if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[Elem->Wcid])) {
				...
				if (wdev) {
					...
					BandIdx = BAND0;
					MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[PeerPublicAction] wdev = NULL\n"));
				}
				pChCtrl = hc_get_channel_ctrl(pAd->hdev_ctrl, BandIdx);
	...
	case ACTION_WIFI_DIRECT:
...
#endif /* IWSC_SUPPORT // */
#ifdef P2P_SUPPORT
		MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<---- Public. Vendor OUI+type = %08x\n", *(PULONG)&Elem->Msg[LENGTH_802_11 + 2]));
#ifdef RT_BIG_ENDIAN
...
#endif /* RT_BIG_ENDIAN */
		{
			...
			MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!!! Public action frames with Vendor specific OUI = WFAP2P.Subtype = %d.\n", Subtype));
			...
			case GO_NEGOCIATION_REQ:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Group Nego. Req\n", __func__));
				P2pPeerGoNegoReqAction(pAd, Elem);
				break;

			case GO_NEGOCIATION_RSP:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Group Nego. Rsp\n", __func__));
				P2pPeerGoNegoRspAction(pAd, Elem);
				break;

			case GO_NEGOCIATION_CONFIRM:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Group Nego. Confirm\n", __func__));
				P2pPeerGoNegoConfirmAction(pAd, Elem);
				break;

			case P2P_INVITE_REQ:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Invite Req\n", __func__));
				P2pPeerInvitesReqAction(pAd, Elem);
				break;

			case P2P_INVITE_RSP:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Invite Rsp\n", __func__));
				P2pPeerInvitesRspAction(pAd, Elem);
				break;

			case P2P_DEV_DIS_REQ:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Device Discovery Req\n", __func__));
				P2pPeerDevDiscoverReqAction(pAd, Elem);
				break;

			case P2P_DEV_DIS_RSP:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Device Discovery Rsp\n", __func__));
				P2pPeerDeviceDiscRspAction(pAd, Elem);
				break;

			case P2P_PROVISION_REQ:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Provision Req\n", __func__));
				P2pPeerProvisionReqAction(pAd, Elem);
				break;

			case P2P_PROVISION_RSP:
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : recv Provision Rsp\n", __func__));
				P2pPeerProvisionRspAction(pAd, Elem);
				break;

			default:
				pAd->P2pCfg.bPeriodicListen = TRUE;
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknown Public action frames with Vendor specific OUI = WFAP2P.Subtype = %d\n", Subtype));
				break;
			}
		}
...
VOID PeerRMAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	case RRM_MEASURE_REQ:
		MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get RRM Measure request.\n",
					__func__));
	...
	case NOTIFY_BW_ACTION:
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if (!pStaCfg || pStaCfg->StaActive.SupportedPhyInfo.bHtEnable == FALSE) {
				...
				MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ACTION -Ignore HT Notify Channel BW when link as legacy mode. BW = %d--->\n",
...
void sta_send_event_report(struct _RTMP_ADAPTER *pAd, struct _MLME_QUEUE_ELEM *Elem)
{
	...
	if (status != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_ERROR, ("WNM event report allocate memory failed\n"));
		return;
	}
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_WNM, DBG_LVL_TRACE, ("WNM event report\n"));
#endif
}
#endif
